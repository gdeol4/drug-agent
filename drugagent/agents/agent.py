...
import anthropic
import drugagent.high_level_actions as high_level_actions
from drugagent.schema import Action, EnhancedJSONEncoder
from drugagent.LLM import complete_text
...
class Agent:
    ...
    def __init__(self, args, env, name):        
        ...
        self.action_infos = env.action_infos_dict[name]
        tool_names = list(env.action_infos_dict[name].keys())
        ...
        self.tools_prompt = self.construct_tools_prompt(tool_names, self.action_infos)
    ...
    def initialize_logging(self): 
        ...
        with open(os.path.join(self.log_dir, f"{self.name}_log"), "w", 1) as f:
            f.write("Enabled Tools in Prompt:" + str(self.prompt_tool_names) + "\n") 
            f.write("================================Start=============================\n")

        print("Agent is up! See progress in {}".format(os.path.join(self.log_dir, f"{self.name}_log")))
